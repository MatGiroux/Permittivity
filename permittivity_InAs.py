...
def eps_FC_I_baltz_calc(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,U): #this is the calculation of the imaginary part of the permittivity at high doping taken from Baltz 1972 https://doi.org/10.1002/pssb.2220510209
...
def eps_FC_I_calc(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,eps_Drude,U): #this model computes the imaginary part of the permittivity at high doping by combining the Baltz and Drude model depending on the region
    ...
    #for the case where w>w_p
    eps_FC_I_2=eps_FC_I_baltz_calc(w_2,E_F,eps_inf,epsilon0,e,N,hbar,m_star,U)
...
def Kramers_Kronig_IB(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,U): #here we perform the Kramers Kronig relations to find the real part of the Interband refractive index
    ...
    w_max=max(max(w),E_max/hbar) #finding the max between the minimum max value for the computation and the max value of the array
    ...
    alpha_IB_KK=alpha_IB_calc(w_vec_KK,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,U)
...
def Kramers_Kronig_FC(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,U): #here we perform the Kramers Kronig relations to find the real part of the free carrier permittivity
    ...
    eps_Drude_KK=eps_inf*(-wp_square/(w_vec_KK*(w_vec_KK+1j*Gamma)))
    #computing the imaginary part of the permittivity
    eps_FC_I_KK=eps_FC_I_calc(w_vec_KK,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,eps_Drude_KK,U)
...
def Hilbert_transform_calc(w_vec_KK,chi_I): #this function computes the Hilbert transform of a data set
...
def eps_FC_R_calc(w,eps_Drude,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,model_choice,U): #this model computes the real part of the permittivity 
    ...
    if model_choice=="Approximation":#if the user chose the approximation to the Kramers-Kronig relation
        ...
        eps_FC_R=Kramers_Kronig_FC(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,U) #computing the real part of the permittivity using the Kramers-Kronig relations
        
    return eps_FC_R


def eps_FCL_calc(w,N,T,eps_inf,m0,mhh,mlh,N_doped,Eg_0_T,kb,hbar,P,model_choice,U):#this function computes the free carrier and lattice response of the permittivity using a Drude-Lorentz model
    ...
    if N_doped==True: #if the sample is N-doped
        if E_F<kb*T:#for the low doping case
            ...
            eps_FC_I=eps_FC_I_calc(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,eps_Drude,U) #computing the imaginary part of the permittivity
            eps_FC_R=eps_FC_R_calc(w,eps_Drude,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,model_choice,U) #computing the real part of the permittivity
...
def eps_IB_calc(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,model_choice,U):#this function computes the Interband response of the permittivity
    ...
    if model_choice=="Approximation":#if the user chose the approximation to the Kramers-Kronig relation
        ...
        m_IB_p=Kramers_Kronig_IB(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,U) #computing the real part of the refractive index using the Kramers-Kronig relations
...
def tot_eps_calc(w,N,T,mhh,mlh,P,eps_inf,m0,hbar,N_doped,kb,model_choice,U):#this function computes the sum of the lattice, free carrier and Interband responses of the permittivty and returns the total permittivity
    
    
    Eg_0_T=Eg_0_T_calc(T,U) #computing the undoped bandgap

    eps_FCL,F=eps_FCL_calc(w,N,T,eps_inf,m0,mhh,mlh,N_doped,Eg_0_T,kb,hbar,P,model_choice,U)#computing the free carrier response of the permittivity
    
    eps_IB=eps_IB_calc(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,model_choice,U)
...
def epsilon_InAs(lambd,N,T,N_doped,model_choice,U):
    ...
    eps_InAs=tot_eps_calc(w,N,T,mhh,mlh,P,eps_inf,m0,hbar,N_doped,kb,model_choice,U)
