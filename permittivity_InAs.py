...
def eps_FC_I_baltz_calc(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,U): 
...
def eps_FC_I_calc(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,eps_Drude,U): 
    '''This function computes the imaginary part of the free-carrier permittivity 
...
def Kramers_Kronig_IB(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,U): 
...
def Kramers_Kronig_FC(w,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,U): 
    '''This function performs the Kramers Kronig relations to find the real part
...
def eps_FC_R_calc(w,eps_Drude,E_F,eps_inf,epsilon0,e,N,hbar,m_star,wp_square,Gamma,model_choice,U): 
    ...
    it to the real part of the Drude model (lead to 1% error).
    '''
...
def eps_FCL_calc(w,N,T,eps_inf,m0,mhh,mlh,N_doped,Eg_0_T,kb,hbar,P,model_choice,U):
    '''This function computes the free carrier and lattice response of the 
    permittivity using a Drude-Lorentz model or a combination of the Drude-Lorentz
    model with the model from Baltz 1972 depending on the doping and temperature
    of the sample.
    '''
...
def kw_calc(w,mhh,m0,Eg_0_T,hbar,P): 
...
def alpha_lh_calc(w,hbar,P,eps_inf,Eg_0_T):
...
def alpha_hh_calc(w,P,kw,Eg_0_T,hbar,m0,mhh,eps_inf):
...
def BM_hh_calc(w,hbar,kb,T,mhh,kw,F): 
...
def BM_lh_calc(w,hbar,kb,T,Eg_0_T,F): 
...
def fermi_calc(N,Eg_0_T,kb,T,P,U):
...
def alpha_IB_calc(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,U):
    ...
    #computing the smooth cutoff function
    h=cutoff_func(w,hbar,U) #we implement a cutoff on alpha_lh since it comes from k.p theory that is valid only for w near the bandgap. This form continues to grow at large w, which makes the K-K relations non-convergent. We apply a smooth cutoff begining at 0.75 eV, which is the energy at which split-off hole absorption would begin to contribute to alpha(w). We apply a cutoff over a range of 1 eV
    ...
    alpha_IB[hbar*w<=Eg_0_T]=0.0
    
    return alpha_IB*U("rad/m")



def eps_IB_calc(w,N,T,eps_inf,mhh,mlh,m0,P,Eg_0_T,F,hbar,N_doped,kb,model_choice,U):
...
def tot_eps_calc(w,N,T,mhh,mlh,P,eps_inf,m0,hbar,N_doped,kb,model_choice,U):
