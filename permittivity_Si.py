...
#The main user-facing function is epsilon_Si. This function computes the dielectric function for Si.
...
def epsilon_Si(w, U, T, ND, NA):
    ...
    eps_bl =eps_bl_calc(U,lambd,T,Eg)
    ...
    Ne,Nh=carrier_concentrations(U,ND,NA,T,T_inf,Eg)
...
def carrier_concentrations(U,ND,NA,T,T_inf,Eg): 
    ...
    Nth=np.sqrt(Nc*Nv*math.exp(-Eg/(U.k*T)))
...
def alpha_3_calc(U,E,Eg,T): 
    ...
    factor=E.m_as("eV")>=(Eg-U.k*theta_3).m_as("eV")
    
    #computing the function
    alpha_3=factor*536*U("eV^(-1)*cm^(-1)")*(E-Eg+U.k*theta_3)**2/(E*(np.exp(theta_3/T)-1))
    
    return alpha_3 #in cm^-1


def alpha_4_calc(U,E,Eg,T): 
    '''This function computes the absorption coefficient associated with phonons 
    ...
    #computing a factor that will act as the equivalent of an if condition (this removes the need for a loop making the code much faster)
    factor=E.m_as("eV")>=(Eg-U.k*theta_4).m_as("eV")
    
    #computing the function
    alpha_4=factor*988*U("eV^(-1)*cm^(-1)")*(E-Eg+U.k*theta_4)**2/(E*(np.exp(theta_4/T)-1))
    
    return alpha_4 #in cm^-1


def alpha_IB_calc(U,lambd,Eg,T): 
    '''This function computes the interband absorption using the model in Timans 1993
    '''
    
    #converting wavelength to energy
    E=2*U.hbar*U.pi*U.c/lambd
    ...
    alpha_3=alpha_3_calc(U,E,Eg,T)
    alpha_4=alpha_4_calc(U,E,Eg,T)
...
def eps_bl_calc(U,lambd,T,Eg): 
    ...
    alpha_IB=alpha_IB_calc(U,lambd,Eg,T)
